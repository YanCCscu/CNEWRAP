      ...
      if( in_analysis == "branch" || in_analysis == "all" )
        {
          ...
          if( anyMissing == 1 ){
            ...
            if( verbose ) logcat("Missing data (genome gaps) were found for that element\n")
...
enoughSpecies = function( elID ){
    ...
    if( verbose ) logcat(paste("Number of species:", length(remSpecies), "\n"))
    if( length(remSpecies) < 2 )
      {
        logcat(paste("    > Only", length(remSpecies), "species left, element skipped\n") )
        return( FALSE )
      }else
        {        
          ...
          if( n1.spec == 0 || n0.spec == 0 ){
            if( n1.spec == 0 ) logcat("    > No species with phenotype 1 left\n", sep="" )
            if( n0.spec == 0 ) logcat("    > No species with phenotype 0 left\n", sep="" )
            return( FALSE )
          }        
    ...
    for( i in unique(branchCatalog$subTree) )
      {
        ...
        else logcat(paste("    > Sub loss tree ", i, " missing\n", sep="" ))            
      }
