...
## Load the libraries
######################################################
pkg=c("phangorn","weights","isotone", "caper") 
for( p in pkg){
...
##                    String, should the conserved branches be collapsed? If 'all', all clades are collapsed, if only some ancestors are specificied (separated by commas without any whitespace), the corresponding clades are collapsed. If 'neutralOnly', the conserved branches are not collapsed, but the branches under neutral evolution are.
  ...
  cat("\n")
  cat("Forward Genomics analysis:\n")
  cat("\n")
  cat(paste("Method(s)"))
  cat(paste("List of elements..........................", in_elements, "\n"))
  cat(paste("Phenotype of all species..................", in_listPheno, "\n"))
  cat(paste("Phylogenetic tree.........................", in_tree, "\n"))
  cat(paste("Output file...............................", out_data, "\n"))
  if (verbose == TRUE)
     cat(paste("Output dir................................", out_path, "\n"))
  ...
  if( in_analysis == "branch" || in_analysis == "all" ){
    ...
    cat(paste("Local %id values..........................", in_localPid, "\n")) 
    cat(paste("Branch weights............................", in_brWeights, "\n"))
    cat(paste("Expected %id values.......................", in_meanPidSel, "\n"))
    cat(paste("%id threshold trait-preserving branches...", in_thresholdConserved, "\n"))
...
##    cat(paste("Collapsing clade..........................", in_collapseClades, "\n"))
  }
  ...
  if( in_analysis == "GLS" || in_analysis == "all" ){
    ...
    ## Display
    cat("\nParameters for GLS:\n")
    cat(paste("Global %id values.........................", in_globalPid, "\n"))
    cat(paste("Min number of independent losses..........", indepLoss, "\n"))
    cat(paste("Raw/normalized............................", in_rawRanked, " (perfect match is computed with raw values only)\n" ))
 }
...
inputFiles = function()
{
  ...
  logcat("Propagate the phenotypes into the tree, spot the loss nodes\n")
  in_lossNodes <<- propagatePheno( tree )
  ## Fill the branch catalog
  logcat("Fill the branch catalog\n")
  ...
  logcat("Check that each tree-leaf has a phenotype\n")
  ...
  if( in_analysis == "GLS" || in_analysis == "all" )
    {
      ...
      logcat( paste( "Removing", length(noPhenotype), "species with unknown phenotype\n" ) )
...
rm.Species = function(){
  ...
  for( s in colnames(in_globalPid) ){
    ...
    if( ! ( s %in% tree$tip.label ) ){
      ...
      logcat( paste( "Species", s, " not found in the tree -> removed from data\n" ) )
    }
  }
}
...
outputFiles = function(){
  ...
  logcat(header, file=nameOutput, sep=" ")
} 
...
for( i in 1:nrow(catalog) ){
  ...
  logcat("Make a data frame from the loss nodes\n")
  vec.lossTime = rep( times=length( vec.lossNodes ), -1 )
  lossNodes = data.frame( vec.lossNodes, vec.lossTime )
  logcat("lossNodes:",vec.lossNodes,"\nlossTime:",vec.lossTime,"\n")
  colnames( lossNodes ) = c( "lossNode", "lossTime" )
  
  ## Display the loss node
  logcat( "\nBranch status\n" )
  capture.output(print(catalog), file = logfile, append = TRUE)
  #print(catalog)
  logcat( "\nLoss nodes\n" )
  #print(lossNodes)
  capture.output(print(lossNodes), file = logfile, append = TRUE)
  return(lossNodes)
}
...
prepareTree = function( thisTree, theseLossNodes ){
        ...
        if( lossNode[[i]] %in% thisTree$node.label )
          {
            ...
            if( lossTime[[i]] > 0 ){
              logcat( paste( "Loss occured", lossTime[[i]], "before", lossNode[[i]], "\n" ) )
              lossNode[[i]] <<- tips.nodes[ Ancestors( x = thisTree, node = which( tips.nodes == lossNode[[i]] ), "parent" ) ]
              logcat( paste( "Switching the loss node to its parent:", lossNode[[i]], "\n\n" ) )
  ...
  options(width=300)
  capture.output(print(thisBranchCatalog), file = logfile, append = TRUE)
...
gatherConservedBranches = function( thisBranchCatalog, thisTree ){
  ...
  for( comb in 1:ncol(combClades) ){
    ...
    if( length( intersect( path1, path2 ) ) > 0 ){
      logcat(paste( "Error in the loss node, two subTrees overlap:", clade1, "and", clade2,"\n") )
      stop()
    }
  }
return( vcvCladeSel )
